...
async function fetchVercelLogs() {
  console.log(
    `Fetching Vercel logs for the last ${TIME_RANGE_MINUTES} minutes...`,
  );
  ...
  try {
    ...
    for (const deployment of deployments) {
      ...
      if (!logsResponse.ok) {
        console.warn(
          `Failed to fetch logs for deployment ${deployment.uid}: ${logsResponse.status}`,
        );
        continue;
      }
...
async function createIssueForError(error) {
  ...
  if (existingIssue) {
    console.log(
      `Issue already exists for error ${fingerprint}: #${existingIssue.number}`,
    );
...
async function main() {
  ...
  try {
    ...
    for (const [fingerprint, errorGroup] of errorsByFingerprint) {
      console.log(
        `Processing error group ${fingerprint} (${errorGroup.length} occurrence(s))`,
      );
