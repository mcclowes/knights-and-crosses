...
export class GameBoard {
    ...
    constructor() {
        this.boardState = {
            results: Array(BOARD_SIZE).fill(0).map(() => Array(BOARD_SIZE).fill(state.EMPTY)),
            frost: Array(BOARD_SIZE).fill(0).map(() => Array(BOARD_SIZE).fill(0)),
            rock: Array(BOARD_SIZE).fill(0).map(() => Array(BOARD_SIZE).fill(0)),
            shields: Array(BOARD_SIZE).fill(0).map(() => Array(BOARD_SIZE).fill(0))
        };
    }
    ...
    private checkDiagonals(): number | undefined {
        ...
        const rightDiagonal = this.boardState.results.reduce((sum, row, i) => sum + row[i], 0);
        if (Math.abs(rightDiagonal) === WIN_CONDITION) {
            return this.boardState.results[0][0];
        }

        // Left-wards diagonal
        const leftDiagonal = this.boardState.results.reduce((sum, row, i) => sum + row[BOARD_SIZE - 1 - i], 0);
    ...
    public isCellEmpty(row: number, col: number): boolean {
        return this.boardState.results[row][col] === state.EMPTY &&
